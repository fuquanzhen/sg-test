...
public class Success {
	...
	// scope 0
	public Success(String path) throws IOException {
		...
		while ((line = br.readLine()) != null) {
			...
			if (map.get(str).size() > 20) {
				List<String> val = map.get(str);
				Collections.shuffle(val);
				List<String> subList = new ArrayList<String>(val.subList(0, 20));
				map.put(str, subList);
			}
		}
	}
	...
	public static void main(String args[]) throws IOException {
		...
		Success q = new Success(path);
		Map<String, List<String>> newmap = q.map;
		Success.write(paths, newmap);
	}
}
